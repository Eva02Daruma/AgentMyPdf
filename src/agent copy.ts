...
export class IntelligentAgent extends Agent<Env, RAGAgentState> {
  ...
  async *askQuestion(question: string, runId?: string): AsyncGenerator<string, RAGResult, unknown> {
    ...
    try {
      yield `🚀 [${finalRunId}] Iniciando pipeline RAG...\n\n`;
      ...
      // Mejorar el prompt con contexto estructurado
      const systemPrompt = `Eres un experto en leyes de Chile.
Tu tarea es responder preguntas basándote ÚNICAMENTE en los documentos legales proporcionados.
...
${documents.map((d, i) => `
[Documento ${i + 1}] (Score: ${d.score})
${d.content}
---`).join('\n')}

Instrucciones:
1. Responde SOLO basándote en los documentos proporcionados
2. Cita los números de documento relevantes
3. Si la pregunta requiere información que no está en los documentos, indícalo
4. Sé específico y menciona artículos o secciones relevantes
...
Respuesta:`;
  ...
  private async *streamAnswer(question: string, documents: Array<{id: string, content: string, source: string, score: number}>): AsyncGenerator<string> {
    const prompt = this.buildPrompt(question, documents.map(d => d.content));
  ...
  /**
   * Construye el prompt con contexto de documentos
   */
